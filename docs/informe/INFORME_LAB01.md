...
> **Respuesta:**  
> El Principio de Responsabilidad Única (SRP) establece que una clase debe tener una sola razón para cambiar, no que deba reducirse a un único método trivial. El anti-patrón "Fantasmas" (Poltergeists) surge cuando un equipo interpreta el SRP de forma literal y fragmenta la lógica en decenas de clases minúsculas (`ValidadorDeEstado`, `CalculadorDeCampo`, `EjecutorDePaso`) cuyo único propósito es invocar a otra clase y desaparecer. En el subsistema de movilidad de este laboratorio, esto equivaldría a descomponer `RentalService.rentVehicle()` en clases como `BuscadorDeVehiculo`, `ValidadorDeDisponibilidad` y `ActualizadorDeEstado`, cada una llamando a la siguiente en cadena.
>
> El problema real es que esta fragmentación no reduce la complejidad: la traslada del código a la topología de llamadas entre objetos. El costo de indirección aumenta porque para entender un flujo de negocio (por ejemplo, un alquiler completo) un desarrollador debe saltar entre múltiples archivos sin lógica de negocio propia, cada uno acoplado rígidamente al siguiente en la cadena. Esto genera **baja cohesión disfrazada de "responsabilidad única"**: las clases no agrupan datos y comportamiento relacionado, sino pasos de un algoritmo artificialmente separados. La inestabilidad se oculta porque, ante un cambio en el flujo (ej. añadir una validación de mantenimiento antes de alquilar), es necesario modificar la cadena completa de clases fantasma, violando en la práctica el mismo OCP y SRP que se pretendía cumplir. Un diseño SRP correcto agrupa responsabilidades cohesivas (como hace `RentalService`, que orquesta el alquiler completo delegando cálculo y persistencia a abstracciones inyectadas), no las atomiza sin sentido.
...
> **Respuesta:**  
> La Ley de LeBlanc resume una observación empírica de la ingeniería de software: **"Later equals never"** ("Después es igual a nunca"). Cuando un desarrollador decide posponer la refactorización de un módulo con la promesa de "lo arreglo después", en la práctica esa limpieza casi nunca ocurre, porque la presión de nuevas funcionalidades siempre tiene mayor prioridad visible para el negocio que la salud interna del código.
>
> En un ecosistema de Smart City, esta postergación es especialmente peligrosa porque la deuda técnica no crece linealmente, sino de forma compuesta: cada nueva funcionalidad urbana (una tarifa festiva, un nuevo tipo de vehículo eléctrico, un protocolo de emergencia adicional) se construye **sobre** el código sucio existente en lugar de sobre una base limpia, heredando y multiplicando sus defectos. Si `TrafficControlCenter` hubiera mantenido un `switch/case` en vez del patrón Strategy (`ITollStrategy`), cada peaje nuevo habría requerido tocar la misma función crítica, incrementando la probabilidad de una regresión. En sistemas urbanos, una regresión no es un bug menor: puede significar semáforos mal calculados, vehículos autónomos con rutas erróneas, o —el caso más grave— una `EmergencyCenter` que falla en despachar una ambulancia por un efecto colateral no detectado en código legado nunca refactorizado. La Ley de LeBlanc advierte que sin disciplina explícita (como aplicar OCP y DIP desde el diseño, tal como se hizo aquí con `PricingStrategy` y `VehicleRepository`), la deuda técnica no se "paga después": se acumula hasta que el costo de cualquier cambio supera el valor del cambio mismo, y el sistema colapsa operativamente.
...
> **Respuesta:**  
> Un "flag argument" es un parámetro booleano (o un enum usado como interruptor) que le indica a una función *qué camino de su lógica interna debe ejecutar*. Por ejemplo, un método hipotético `controlSemaforo(boolean esModoEmergencia)` en realidad no hace "una sola cosa": hace dos cosas distintas (operar en modo normal u operar en modo emergencia) empaquetadas bajo un único nombre de función, y el llamador debe conocer el significado interno de `true`/`false` para predecir el comportamiento —lo cual ya es una violación de la honestidad en los nombres (5S Seiri) descrita en la introducción de esta guía.
>
> Esto viola directamente el principio de "Hacer una sola cosa" (Clean Code, Martin, 2009) porque obliga a la función a bifurcar su flujo internamente con un `if (esModoEmergencia) { ... } else { ... }`, generando dos responsabilidades cohabitando el mismo bloque de código, cada una con su propio conjunto de casos de prueba mezclados. La legibilidad se degrada en el sitio de la llamada: `controlSemaforo(true)` no comunica intención, mientras que `activarModoEmergencia()` y `activarModoNormal()` sí lo hacen. La solución consistente con el resto de este laboratorio es la misma que se aplicó con el patrón Strategy en `TrafficControlCenter`: en vez de un booleano que bifurca el comportamiento, se definen funciones (o estrategias) explícitas y independientes para cada modo, permitiendo que cada una sea leída, probada y modificada sin interferir con la otra, y evitando que el crecimiento de casos futuros (ej. "modo festivo", "modo nocturno") derive en una cadena de flags booleanos anidados.
